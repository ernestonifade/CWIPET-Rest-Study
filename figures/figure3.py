...
def render_pathway_enrichment_bubble_from_df(
    ...
    if df.empty:
        st.warning(
            "⚠️ No pathways meet the significance threshold (FDR < 0.05)."
        )
        return None
    ...
    fig, ax = plt.subplots(figsize=(3,3))#(figsize=(4.0, max(4.0, len(df) * 0.25)))
    ...
    if selected_view == "🧬 Pathway Enrichment: Correlating Metabolites":
        ...
        st.markdown(
            ...
            <div style="background-color: #fff3cd; border-left: 4px solid #ffc107; padding: 10px 14px; margin-bottom: 12px; border-radius: 4px; font-size: 12px; line-height: 1.5; color: #856404;">
                <b>📄 Metabolite Pathway Summary Table:</b> Complete statistical enrichment metrics for metabolite correlates (Top 15).
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            """
            <div style="background-color: #fff3cd; border-left: 4px solid #ffc107; padding: 10px 14px; margin-bottom: 12px; border-radius: 4px; font-size: 12px; line-height: 1.5; color: #856404;">
                <b>📄 Cytokine Pathway Summary Table:</b> Complete statistical enrichment metrics for cytokine correlates (Top 15).
            </div>
            """,
            unsafe_allow_html=True,
        )
