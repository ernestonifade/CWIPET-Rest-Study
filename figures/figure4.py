...
def render_figure4():
    ...
    if selected_view == '🔥 Heatmap: Time × Group Interactions Effect':
        ...
        <div style="background-color: #fff3cd; border-left: 4px solid #ffc107; padding: 10px 14px; margin-bottom: 12px; border-radius: 4px; font-size: 11px; color: #856404;">
            <b>Figure 4A (Candidate Heatmap):</b> Relative fold change dynamics for candidate proteins meeting nominal significance (<i>p</i><sub>raw</sub> &lt; 0.05) for Time × Group interaction across recovery.
        </div>
        """, unsafe_allow_html=True)

        fig_hm = plot_interaction_heatmap_19_proteins(
            long_df=fig4_long_df,
        ...
        # Subplot 1: Muscle Temp1
        sub_size1 = df_emm[df_emm["Metabolite"] == "Muscle Temp1"]
        for idx, row in sub_size1.iterrows():
            group_val = row["Group"]
            ...
            axes[0].errorbar(
                x_pos, row["EMM"],
                yerr=[[row["EMM"] - row["CI_lower"]], [row["CI_upper"] - row["EMM"]]],
                fmt=markers[x_pos], color=colors[x_pos], capsize=5, markersize=7
            )
        axes[0].set_xticks([0, 1, 2])
        axes[0].set_xticklabels(group_order)
        axes[0].set_xlim(-0.5, 2.5)
        axes[0].set_ylabel("Adjusted EMM (°C)")
        axes[0].set_title("Muscle Temp 1")
        axes[0].grid(axis="y", linestyle="--", alpha=0.5)

        # Subplot 2: Muscle Temp2
        sub_size2 = df_emm[df_emm["Metabolite"] == "Muscle Temp2"]
        for idx, row in sub_size2.iterrows():
            group_val = row["Group"]
            if group_val in group_order:
                x_pos = group_order.index(group_val)
            else:
        ...
        axes[1].set_ylabel("Adjusted EMM (°C)")
        axes[1].set_title("Muscle Temp 2")
        axes[1].grid(axis="y", linestyle="--", alpha=0.5)

        # Subplot 3: Muscle Temp3
        sub_size3 = df_emm[df_emm["Metabolite"] == "Muscle Temp3"]
        for idx, row in sub_size3.iterrows():
            group_val = row["Group"]
            if group_val in group_order:
                x_pos = group_order.index(group_val)
            else:
        ...
        axes[2].set_ylabel("Adjusted EMM (°C)")
        axes[2].set_title("Muscle Temp 3")
