...
def add_bracket(ax, x1, x2, y, h, text):
    ...
    lx, rx = x1, x2
    ax.plot([lx, lx, rx, rx], [y, y + h, y + h, y], lw=1.2, c='k', clip_on=False)
    ax.text((lx + rx) * 0.5, y + h + 0.2, text, ha='center', va='bottom', color='k', fontsize=7, fontweight='bold')
...
def load_results():
    ...
    metadata_cols = [
        'Subject_ID', 'subject_id', 'ID', 'id',
        'sex', 'Sex', 'GROUP', 'Group', 'group',
...
def render_figure1():
    ...
    if selected_view == '🔥 Heatmap: Time × Group Interactions Effect':
        ...
        df_emm = pd.DataFrame({
            'Metabolite': [
                'IL-26', 'IL-26', 'IL-26',
                'Aminomalonic acid', 'Aminomalonic acid', 'Aminomalonic acid',
                'IL-11', 'IL-11', 'IL-11',
                'Palmitelaidic acid', 'Palmitelaidic acid', 'Palmitelaidic acid',
            ],
            'Group': [
                '22°C', '15°C', '8°C',
                '22°C', '15°C', '8°C',
        ...
        if not df_ancova_fmt.empty:
            ...
            df_ancova_fmt['p_value_raw_fmt'] = df_ancova_fmt['p_value_raw'].apply(
                lambda p: f'{p:.4f}' if pd.notnull(p) and p >= 0.0001 else ('< 0.0001' if pd.notnull(p) else 'N/A')
            )
        ...
        for eff_key, eff_title in effect_map:
            if eff_key == 'TimePoint:Group':
                sub = df_ancova_fmt[
                    (df_ancova_fmt['Effect'].str.contains('TimePoint:Group', case=False, na=False))
        ...
        between_sub = df_ph_fmt[
            (df_ph_fmt['contrast'].str.contains('Male|Female|22°C|8°C', case=False, na=False))
