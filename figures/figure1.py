...
def add_bracket(ax, x1, x2, y, h, text):
    """Draws a significance bracket between two x-positions on a matplotlib axis."""
    lx, rx = x1, x2
    ax.plot(
        [lx, lx, rx, rx],
        [y, y + h, y + h, y],
...
def load_results():
    ...
    protein_cols = [
        ...
        if c not in meta_in_df and pd.api.types.is_numeric_dtype(wide_df[c])
    ]
...
def plot_interaction_heatmap_19_proteins(
    ...
    long_fc['col'] = (
        long_fc['time'].map(time_display).fillna(long_fc['time'])
        + '_'
...
def render_figure1():
    ...
    if selected_view == '🔥 Heatmap: Time × Group Interactions Effect':
        st.markdown(
            """
        ...
        plt.suptitle(
            'Estimated Marginal Means (Baseline Adjusted)', fontweight='bold'
        )
        st.pyplot(fig)
        ...
        search_query = st.text_input(
            '🔍 Search Protein / Cytokine Name:',
            key='unique_ancova_search_input',
        ).strip()
        ...
        if search_query:
            col_name = (
                'Protein'
                if 'Protein' in df_ancova_fmt.columns
                else (
                    'Cytokine' if 'Cytokine' in df_ancova_fmt.columns else None
                )
            )
            if col_name:
                df_ancova_fmt = df_ancova_fmt[
                    df_ancova_fmt[col_name]
                    .astype(str)
        ...
        if not df_ancova_fmt.empty:
            ...
            df_ancova_fmt['Partial_Eta_Squared'] = df_ancova_fmt[
                'Partial_Eta_Squared'
            ].round(3)
            df_ancova_fmt['p_value_raw_fmt'] = df_ancova_fmt['p_value_raw'].apply(
                lambda p: f'{p:.4f}'
                if pd.notnull(p) and p >= 0.0001
                else ('< 0.0001' if pd.notnull(p) else 'N/A')
            )
            df_ancova_fmt['p_value_FDR_fmt'] = df_ancova_fmt['p_value_FDR'].apply(
                lambda p: f'{p:.4f}'
                if pd.notnull(p) and p >= 0.0001
                else ('< 0.0001' if pd.notnull(p) else 'N/A')
            )
        ...
        effect_map = [
            (
                'TimePoint:Group',
                '1. Time × Group Interaction Effects (p_raw < 0.05)',
            ),
        ...
        rename_dict = {
            'p_value_raw_fmt': 'p_value_raw',
            'p_value_FDR_fmt': 'p_value_FDR',
        }
        ...
        for eff_key, eff_title in effect_map:
            if eff_key == 'TimePoint:Group':
                sub = df_ancova_fmt[
                    (
                        df_ancova_fmt['Effect'].str.contains(
                            'TimePoint:Group', case=False, na=False
                        )
                    )
                    & (df_ancova_fmt['p_value_raw'] < 0.05)
                ].sort_values('p_value_raw')
            else:
                sub = df_ancova_fmt[
                    (df_ancova_fmt['Effect'] == eff_key)
                    & (df_ancova_fmt['p_value_raw'] < 0.05)
                ].sort_values('p_value_raw')

            st.markdown(
                f'<h4 style="margin-top:22px; margin-bottom:6px; color:#2c3e50;">{eff_title}</h4>',
                unsafe_allow_html=True,
            )
            if not sub.empty:
                st.dataframe(
                    sub[cols_to_show].rename(columns=rename_dict),
                    use_container_width=True,
                )
            else:
                st.markdown(
                    '<p style="font-size:11px; color:#7f8c8d;'
                    ' font-style:italic;">No proteins met nominal significance (p_raw'
                    ' &lt; 0.05) for this effect.</p>',
                    unsafe_allow_html=True,
                )

    elif selected_view == '🔍 Post-Hoc Pairwise Contrasts (emmeans)':
        search_query = st.text_input(
            '🔍 Search Protein / Cytokine Name:', key='posthoc_search_bar'
        ).strip()
        ...
        if search_query:
            col_name = (
                'Protein'
                if 'Protein' in df_ph_fmt.columns
                else (
                    'Cytokine' if 'Cytokine' in df_ph_fmt.columns else None
                )
            )
            if col_name:
                df_ph_fmt = df_ph_fmt[
        ...
        if not df_ph_fmt.empty:
            for col in ['estimate', 'std_error', 'df', 't_ratio']:
                if col in df_ph_fmt.columns:
                    df_ph_fmt[col] = pd.to_numeric(
                        df_ph_fmt[col], errors='coerce'
                    ).round(3)

            df_ph_fmt['p_value_raw_fmt'] = df_ph_fmt['p_value_raw'].apply(
                lambda p: f'{p:.4f}'
                if pd.notnull(p) and p >= 0.0001
                else ('< 0.0001' if pd.notnull(p) else 'N/A')
            )
            df_ph_fmt['p_value_FDR_fmt'] = df_ph_fmt['p_value_FDR'].apply(
                lambda p: f'{p:.4f}'
                if pd.notnull(p) and p >= 0.0001
                else ('< 0.0001' if pd.notnull(p) else 'N/A')
            )
        ...
        between_sub = df_ph_fmt[
            (
                df_ph_fmt['contrast'].str.contains(
                    'Male|Female|22°C|8°C', case=False, na=False
                )
            )
            & (df_ph_fmt['p_value_raw'] < 0.05)
        ].sort_values('p_value_raw')

        st.markdown(
            '<h4 style="margin-top:15px; margin-bottom:6px;'
            ' color:#2c3e50;">1. Between-Group Pairwise Contrasts (Male vs. Female'
            ' by TimePoint)</h4>',
            unsafe_allow_html=True,
        )
        if not between_sub.empty:
            st.dataframe(
                between_sub[existing_cols].rename(columns=ph_rename),
                use_container_width=True,
            )
        else:
            st.markdown(
                '<p style="font-size:11px; color:#7f8c8d;'
                ' font-style:italic;">No between-group contrasts met nominal'
                ' significance (p_raw &lt; 0.05).</p>',
                unsafe_allow_html=True,
            )

        within_sub = df_ph_fmt[
            (
                ~df_ph_fmt['contrast'].str.contains(
                    'Male|Female|22°C|8°C', case=False, na=False
                )
        ...
        st.markdown(
            '<h4 style="margin-top:25px; margin-bottom:6px;'
            ' color:#2c3e50;">2. Within-Group Pairwise Contrasts (Recovery Time'
            ' Shifting)</h4>',
            unsafe_allow_html=True,
        )
        if not within_sub.empty:
            st.dataframe(
                within_sub[existing_cols].rename(columns=ph_rename),
                use_container_width=True,
            )
        else:
            st.markdown(
                '<p style="font-size:11px; color:#7f8c8d;'
                ' font-style:italic;">No within-group contrasts met nominal'
                ' significance (p_raw &lt; 0.05).</p>',
                unsafe_allow_html=True,
            )